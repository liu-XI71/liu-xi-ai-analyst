...
const valueLabels={current:'本期',previous:'对比期',control:'对照组',treatment:'处理组',holdout:'留出组',first:'近期单次购买',active:'活跃复购',cooling:'31—60 日未购',dormant:'60 日以上未购',single:'单次购买沉默',completed:'完成',needs_clarification:'需澄清',insufficient_data:'数据不足',paid:'付费渠道',organic:'自然渠道',referral:'推荐渠道',ios:'iOS',android:'Android',web:'网页'};
...
export function tableHTML(table, maxRows = 100){
  ...
  return `<div class="data-table-wrapper"><table class="data-table"><thead><tr>${columns.map(c=>`<th scope="col" title="${escape(c)}">${escape(label(c))}</th>`).join('')}</tr></thead><tbody>${rows.slice(0,maxRows).map(row=>`<tr>${columns.map((c,i)=>{const v=Array.isArray(row)?row[i]:row[c];return `<td title="${escape(valueText(v))}">${escape(typeof v==='number'?formatNumber(v,4):valueText(displayValue(c,v)))}</td>`;}).join('')}</tr>`).join('')}</tbody></table></div>${rows.length>maxRows?`<p class="field-help">展示前 ${maxRows} 行，共 ${rows.length} 行；完整结果可在报告中查看。</p>`:''}`;
}
