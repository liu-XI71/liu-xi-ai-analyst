...
尚有 16000 名用户未完成预注册观察窗口，不能把未观测结果计为 0 或剔除后发布总体结论。

## 决策备忘录

```json
{
  "code": "waiting_for_maturity",
  "label": "等待成熟",
  "reason": "尚有 16000 名用户未完成预注册观察窗口，不能把未观测结果计为 0 或剔除后发布总体结论。",
  "action": "等待最后入组用户的 D7 完整结束；最早数据完整日为 2026-09-13，补齐后复查。",
  "gates": {
...
- 等待成熟：等待最后入组用户的 D7 完整结束；最早数据完整日为 2026-09-13，补齐后复查。（证据：experiment-registry, experiment-quality, experiment-changes, experiment-groups）
...
| 分配比例 SRM | 对照:处理=50%:50%；卡方 p≥0.001，期望人数均≥5 | 0.39320751955611144 | True |
| 固定观察窗口成熟 | 全部分配用户已完整观察 D7 与 24h 围栏；入组窗口已结束 | 16000 | False |
...
```
参数：{"experiment_id": "immature", "start": "2026-09-03", "end": "2026-09-06", "cutoff": "2026-09-07 00:00:00", "config_version": "onboarding-v1"}
...
- evaluate_experiment：等待成熟；尚有 16000 名用户未完成预注册观察窗口，不能把未观测结果计为 0 或剔除后发布总体结论。
