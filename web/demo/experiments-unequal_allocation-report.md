...
运行编号：cd3be8fc13fcec07ce14c68fb491cebc · 模式：demo · 时间：2026-09-19T09:24:37.144573+00:00

**数据说明：合成演示数据，不代表任何企业真实经营结果。**

处理组精确 D7 留存 33.30%，对照组 28.31%；差异 +4.98 pp，95% 区间 [3.60, 6.35] pp。满足预设条件，建议人工评审灰度。质量、成熟、设计样本、业务效应区间与体验非劣围栏均通过。

## 决策备忘录

```json
{
  "code": "review_for_gradual_rollout",
  "label": "满足预设条件，建议人工评审灰度",
  "reason": "质量、成熟、设计样本、业务效应区间与体验非劣围栏均通过。",
...
```json
{
  "experiment_id": "unequal_allocation",
  "start_date": "2026-08-17",
  "end_date": "2026-08-30",
  "snapshot_date": "2026-09-06",
  "expected_treatment_share": 0.7,
  "baseline_rate": 0.28,
  "mde": 0.025,
  "alpha": 0.05,
  "power": 0.8,
  ...
  "expected_assignment_count": 20000,
...
```json
{
  "control_required": 3712,
  "treatment_required": 8662,
  "primary_control_required": 3712,
  "primary_treatment_required": 8662,
  "guardrail_control_required": 1914,
  "guardrail_treatment_required": 4466,
  "total_required": 12374,
  "allocation_treatment_to_control": 2.333333333333333,
...
- 入组窗口 2026-08-17 至 2026-08-30，数据完整截至 2026-09-06；共 20000 名分配用户，0 名尚未完成观察。（证据：experiment-registry, experiment-quality）
- 处理组精确 D7 留存 33.30%，对照组 28.31%；差异 +4.98 pp，95% 区间 [3.60, 6.35] pp。（证据：experiment-groups, experiment-registry）
- 负反馈差异 -0.11 pp，非劣评审上界 0.47 pp，预注册容忍界值 1.50 pp。（证据：experiment-groups, experiment-registry）
...
- 满足预设条件，建议人工评审灰度：由业务与实验负责人复核适用人群、实施成本和发布风险，再决定灰度范围及回滚阈值；本系统不执行推全。（证据：experiment-registry, experiment-quality, experiment-changes, experiment-groups）
...
| --- | --- | --- | --- | --- | --- |
| control | 6149 | 6149 | 5541 | 1741 | 254 |
| treatment | 13851 | 13851 | 12516 | 4612 | 557 |
### 预注册样本规划
...
| --- | --- | --- | --- | --- | --- | --- |
| 精确 D7 提升 | 28.000000000000004 | 2.5 | 0.05 | 0.8 | 3712 | 8662 |
| 负反馈非劣围栏 | 4.0 | 1.5 | 0.025 | 0.8 | 1914 | 4466 |
### 配置变更轨迹
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 精确 D7 留存 | 28.313547 | 33.297235 | 4.983688 | 17.601779 | 3.602871 | 6.347587 | 0.0 |
| 24h 负反馈 | 4.130753 | 4.02137 | -0.109383 | -2.648008 | -0.722902 | 0.470312 | 0.717439 |
### 实验评审门槛
...
| --- | --- | --- | --- |
| 来源人数对账 | 分配日志条数等于源端清单 | 20000 / 20000 | True |
...
| 分配比例 SRM | 对照:处理=30%:70%；卡方 p≥0.001，期望人数均≥5 | 0.021498130968659587 | True |
| 固定观察窗口成熟 | 全部分配用户已完整观察 D7 与 24h 围栏；入组窗口已结束 | 0 | True |
| 达到设计样本 | 对照≥3712、处理≥8662；由 MDE/功效和围栏共同规划 | 对照 6149 / 处理 13851 | True |
| 主要指标与业务门槛 | D7 差异双侧 95% 区间下界≥1 pp | [3.603, 6.348] pp | True |
| 负反馈非劣围栏 | 负反馈差异单侧 97.5% 上界<1.5 pp | 上界 0.470 pp | True |
...
| --- | --- | --- |
| 对照组 | 28.313546918198078 | 4.13075296796227 |
| 处理组 | 33.297234856689045 | 4.021370298173417 |
...
```
参数：{"experiment_id": "unequal_allocation"}
...
```
参数：{"experiment_id": "unequal_allocation", "start": "2026-08-17", "end": "2026-08-30", "cutoff": "2026-09-07 00:00:00", "config_version": "onboarding-v1"}
...
- evaluate_experiment：满足预设条件，建议人工评审灰度；质量、成熟、设计样本、业务效应区间与体验非劣围栏均通过。
...
```json
{
  ...
  "snapshot_sha256": "cd3be8fc13fcec07ce14c68fb491cebc5dadae25156c4a620813943fae6a251e",
