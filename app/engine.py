...
DOMAINS={'growth':growth,'repurchase':repurchase}
...
def analyze_domain(request:dict, *,infer=True)->dict:
    ...
    domain=request.get('domain','growth')
    if domain not in DOMAINS:raise ValueError('未知分析场景。')
    q=request.get('question','')
    if re.search(r'(?<![A-Za-z0-9])D\s*\d+(?![A-Za-z0-9])|第\s*[0-9一二三四五六七八九十百]+\s*[天日]\s*(?:的\s*)?(?:精确\s*)?(?:留存|复购|回访)',q,re.I):
    ...
    request['task']=task
    if infer and not any(x in q.lower() for x in ['留存','增长','渠道','设备','实验','报告','周报','分群','客户','名单','复购','预算','留出','召回','转化','ab test','retention']):
...
def business_context(domain:str)->str:
    folder=ROOT/'skills'/('growth-diagnosis' if domain=='growth' else 'repurchase-operations')
    parts=[]
...
def evaluation_summary():
    ...
    for filename,key in [('audit-results.json','audit_summary'),('first-run.json','first_run_summary')]:
